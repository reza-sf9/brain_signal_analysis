function out = removed_fft_mean_rsf(fft_win_chunk , config)
...
pp = size(fft_win_chunk);

out = cell(1, pp(2));

for k = 1 : pp(2)
    
    temp_fft_win_chunk = fft_win_chunk{k};
    
    % number of chunk windows
    m = size(temp_fft_win_chunk);
    
    fft_chunk_mean_removed = cell(1,m(2));
    for ch_num = 1 : m(2)
        fft_chunk = cell2mat(temp_fft_win_chunk(1,ch_num));
