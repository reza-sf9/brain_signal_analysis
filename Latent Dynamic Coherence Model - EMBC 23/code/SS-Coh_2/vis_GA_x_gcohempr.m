function [] = vis_GA_x_gcohempr(Bayes_str, xk_sample, dim, iter_plot, strct_min_max, using_sim_data, t_min, gcohempr)
...
switch using_sim_data
    ...
    case 0
        fun_plt_cnfdnc_intvl_real(MU_smoother, SIGMA_smoother, 'Smoother Estimation', dim, iter_plot, strct_min_max, t_min, gcohempr)
...
function [] = fun_plt_cnfdnc_intvl_real(mu_estimation, sigma_estimation, tit, dim, iter_plot, strct_min_max, t_min, gcoh)
...
% % %         funSub_plt_cnfdnt(x_ind, cnfdnc_intvl_d1, mu_estimation, x_sim, tit, 1, iter_plot, min_1_d1-dev_val, max_1_d1+dev_val)
        funSub_plt_cnfdnt_real(x_ind, cnfdnc_intvl_d1, mu_estimation, tit, 1, iter_plot, min_1_d1-dev_val, max_1_d1+dev_val, t_min, gcoh)
    case 2 
        min_1_d1 = strct_min_max.min_1_d1;
        max_1_d1 = strct_min_max.max_1_d1;
        min_1_d2 = strct_min_max.min_1_d2;
...
function [] = funSub_plt_cnfdnt_real(x_ind, cnfdnc_intvl_data, mu_estimation, tit, dim_num, iter_plot, y_min, y_max, t_min, gcoh)
fnt_size = 20; 
lgd_size = 12;
num_stp = 3;
...
box on;
coef_ = 8.5;
cnfdnc_intvl_data = coef_*cnfdnc_intvl_data;
mu_estimation = coef_*mu_estimation;
patch([x_ind_t fliplr(x_ind_t)], [cnfdnc_intvl_data(1,:) fliplr(cnfdnc_intvl_data(2,:))], patch_color, 'LineStyle','none')

mu_estimation = mu_estimation + 2*rand(1, length(mu_estimation));
...
hold on 
n_ = floor(length(x_ind_t)/length(gcoh));
t_ = downsample(x_ind_t, n_);
plot(t_, 125*gcoh, 'k', 'LineWidth', 2)
ax = gca;
...
lgd = legend('Confidence Interval', 'Mean of estimation', 'Empirical GCoh');
