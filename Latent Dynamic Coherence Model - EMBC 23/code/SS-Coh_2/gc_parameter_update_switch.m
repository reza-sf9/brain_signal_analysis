function [Param_Update, error_tot] = gc_parameter_update_switch(Param, ParamaUpdate, Bayes, Yk, iter, W_sim, update_coef_W, mat_switch_active_ind)
...
if sigma_update == 1
    % estimate sv2
    [Q_estimate, error_tot] = gc_update_sigma_switch(Bayes, F, Q, dim, mat_switch_active_ind);
...
if (mu_update+W_update+L_update) >= 1
    thr_val = 1e-6;
    count_break = 1;
    [mu_estimate, L_estimate, W_estimate, error_tot] = gc_update_mu_L_W_switch(ParamaUpdate, Param, Bayes, Yk, thr_val, count_break, iter, W_sim, update_coef_W, dim, mat_switch_active_ind);
