function [W_estimate, dif_W] = gc_estimate_W_2d_1_1(MU_smoother, SIGMA_smoother, Yk, mu, W, L, incongruent_vec, iter, W_sim, update_coef_W, dim)
...
for m=1: ch_num
    ...
    Um = L(:, m);
    updata_vec = update_coef_W(m, :);
    
        %     disp(m)
        
        % check we want to update or not
        updata_val = updata_vec(1);
        ...
        if updata_val==1
            ...
            if dim==1
                ...
                x0 = [Wm1 Wm3]; % initial guess for solution
                
                % call find min function
                x = fminsearch(@(x)f_update_wm1_1d(x, MU_smoother, SIGMA_smoother, Yk, mu, Um),x0);
...
function obj = f_update_wm1_1d(x, MU_smoother, SIGMA_smoother, Yk, mu, Um)
...
for k=1: K
    ...
    vec_0 = (mu_s-.5*sigma_s*Wm_hat);
    vec_1 = [vec_0 1].';
    
    % estimate E[lambda_inv] - Eq 4.c (UPDATE_L_W.docx)
    Exp_term = exp(-(Wm_hat_wm3.' * vec_1));
