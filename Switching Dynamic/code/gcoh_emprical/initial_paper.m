...
str_save_yk = sprintf('switch_data_Yk_chNum%d_fr%d_sr%d_winSec%d_segNum%d_TRIAL.mat',config.ch_num, fr, sr, config.win_sec, config.seg_num);% using 2 tasks
save(str_save_yk, 'Y_k')

str_save = sprintf('switch_data_cfg_Init_chNum%d_fr%d_sr%d_winSec%d_segNum%d_TRIAL.mat',config.ch_num, fr, sr, config.win_sec, config.seg_num); % using 2 tasks
