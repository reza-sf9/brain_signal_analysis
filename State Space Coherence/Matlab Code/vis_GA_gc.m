...
plot(x_ind_t_empirical, GC_empirical, 'LineWidth', 2, 'color', 'k')
...
for k=1:K
    gc_mean_temp = gc_mean(k)+.17;
    gc_upper_temp = gc_upper(k)+.25;
    gc_lower_temp = gc_lower(k)+.25;
    ...
    bias_ = 0.5;
    GC_img(ind_min+1: ind_min+x_dif_upper(end), k) = y_dff_upper+bias_;
    ...
    GC_img(ind_min-1: -1:ind_min - x_dif_lower(end), k) = y_dff_lower+bias_;
